...
class CheckPointer(object):
    def __init__(self, args, model=None, optimizer=None):
        ...
        trgset       = args['data.trgset']
        if isinstance(train_set,list):
            train_set = train_set[0]
        if isinstance(trgset, list):
            trgset = trgset[0]
        # if train_set == 'ilsvrc_2012':
        #     train_set = 'imagenet'
        # train_set = train_set+'-net'

        self.root_dir       = args['model.save_dir']
        self.model_root_dir = os.path.join(self.root_dir,trgset)
        # self.model_root_dir = self.root_dir
        self.model_path = os.path.join(self.model_root_dir,train_set)

        # self.model_path = os.path.join('/mfs/xxzhang/Fewshot_Selection/',train_set)
    ...
    def save_checkpoint(self, epoch, best_val_acc, best_val_loss,
        ...
        model_path = check_dir(self.model_path, True)
