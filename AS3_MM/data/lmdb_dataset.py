...
class LMDBDataset:
    ...
    """
    def __init__(self, args,extractor_domains, datasets, backbone, mode, dump_name, limit_len=None):
        self.mode     = mode
        self.datasets = datasets
        self.trgset   = args['data.trgset'][0]
        # self.trgset = 'cu_birds'
        ...
        for evalset in datasets:
            ...
            model_dir = args['model.save_dir']
            all_names = os.listdir(os.path.join(model_dir,self.trgset,'Dumps',backbone,mode,evalset))
            # all_names = os.listdir(os.path.join(model_dir,'features_db', self.trgset, 'Dumps', backbone, mode, evalset))
            self.dataset_readers[evalset] = [
                DatasetReader(args,extractor_domains, evalset, backbone, mode, name)
...
class DatasetReader(object):
    ...
    """
    def __init__(self,args,extractor_domains, evalset, backbone, mode, name):
        self._mode = mode
        self._env = None
        self._txn = None
        self.keys = None
        ...
        model_dir = args['model.save_dir']
        path = os.path.join(model_dir,self.trgset,'Dumps',backbone,mode,evalset,name)
        # path = os.path.join(model_dir,'features_db',self.trgset,'Dumps',backbone,mode,evalset,name)
