...
class MetaDatasetReader(object):
    def __init__(self, mode,dataspec_dir, train_set, validation_set, test_set):
        assert (train_set is not None or validation_set is not None or test_set is not None)

        self.data_root_path = META_RECORDS_ROOT
        self.dataspec_dir = dataspec_dir
    ...
    def _get_dataset_spec(self, items):
        if isinstance(items, list):
            ...
            for dataset_name in items:
                ...
                data_pth = self.data_root_path + dataset_name
                dataset_spec_pth = self.dataspec_dir + '/' + dataset_name + '_dataset_spec.json'
                dataset_spec = dataset_spec_lib.load_dataset_spec(dataset_spec_pth,data_pth)
    ...
    def num_classes(self, split_name):
        ...
        for specs in all_split_specs:
            ...
            total_n_classes += len(specs.get_classes(split,specs_dict))
    ...
    def build_class_to_identity(self):
        ...
        for dataset_id, specs in enumerate(all_split_specs):
            ...
            specs_dict = specs.to_dict()
            cats = specs.get_classes(split,specs_dict)
            self.dataset_to_n_cats[dataset_name] = len(cats)
            for cat in cats:
                self.cls_to_identity[cat] = (cat, dataset_id)
            # offset += len(cats)
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    """
    def __init__(self, mode,dataspec_dir, train_set=None, validation_set=None, test_set=None):
        super(MetaDatasetEpisodeReader, self).__init__(mode, dataspec_dir, train_set, validation_set, test_set)
    ...
    def _get_task(self, next_task, session):
        ...
        task_dict = {
            ...
            'context_class_ids':episode[2],
            'target_images': episode[3],
            'target_labels': episode[4],
            'target_class_ids':episode[5]
            }
...
class MetaDatasetBatchReader(MetaDatasetReader):
    ...
    Class that wraps the Meta-Dataset episode readers.
    """
    def __init__(self, mode,dataspec_dir, train_set, validation_set, test_set, batch_size):
        super(MetaDatasetBatchReader, self).__init__(mode, dataspec_dir,train_set, validation_set, test_set)
    ...
    def _init_multi_source_dataset(self, items, split):
        ...
        self.class_set = []
        for set_name in items:
            dataset_spec = self._get_dataset_spec(set_name)
            dataspec_dict = dataset_spec.to_dict()
            class_set = dataset_spec.get_classes(SPLIT_NAME_TO_SPLIT['train'], dataspec_dict)
            self.class_set.append(class_set)
    ...
    def _init_single_source_dataset(self, dataset_name, split):
        ...
        iterator = multi_source_pipeline.make_one_shot_iterator()
        return iterator.get_next()
    def re_label(self,labels,unique_labels):
        re_labels = []
        for l in labels:
            new_label = np.where(l==unique_labels)[0][0]
            re_labels.append(new_label)
        re_labels = np.array(re_labels)
        return re_labels

    def _get_batch(self, next_task, session,tag='train'):
        ...
        if tag == 'train':
            unique_labels = self.class_set[source_id]
            labels = self.re_label(labels,unique_labels)
    ...
    def get_validation_batch(self, item, session):
        return self._get_batch(self.validation_set_dict[item], session,tag='val')

    def get_test_batch(self, item, session):
        return self._get_batch(self.test_set_dict[item], session,tag='test')
