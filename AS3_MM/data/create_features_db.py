...
class DatasetWriter(object):
    def __init__(self, args, rewrite=True, write_frequency=10):
        ...
        self.save_model_root_dir = args['model.save_dir']
        self.dataset_models      = DATASET_MODELS_DICT[args['model.backbone']]
        # print(self.dataset_models)

        dataspec_dir                 = f'{args["data.dataspec_root_dir"]}/{self.trgset}'
        ...
        path = check_dir(os.path.join(self.save_model_root_dir,self.trgset,'Dumps',self.args['model.backbone'],
                                      self._mode,evalset,dump_name))
        # path = check_dir(os.path.join(self.save_model_root_dir,'features_db', self.trgset, 'Dumps', self.args['model.backbone'],
        #                               self._mode, evalset, dump_name))
        self._path = path
    ...
    def encode_dataset(self, n_tasks=1000):
        ...
        with tf.compat.v1.Session(config=config) as session:
            for idx in tqdm(range(n_tasks)):
                ...
                keys_values =  list(np.unique(sample['context_labels'].detach().cpu().numpy()))
