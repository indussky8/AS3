...
class EpisodeDescriptionSampler(object):
  ...
  def __init__(self,
    ...
    self.dataset_spec_dict = dataset_spec.to_dict()
    self.class_set = dataset_spec.get_classes(self.split,self.dataset_spec_dict)
    ...
    if self.use_bilevel_hierarchy:
      ...
      if self.split == SPLIT_NAME_TO_SPLIT['train']:
          self.superclass_set = []
          all_superclasses = dataset_spec.episode_train_get_supclass(split,self.dataset_spec_dict)
          for i in all_superclasses:
            if dataset_spec.episode_train_get_supclass_per_class_number(split,self.dataset_spec_dict,i) > self.min_ways:
                # raise ValueError(
  ...
  def sample_class_ids(self):
    ...
    if self.use_dag_hierarchy and RNG.choice([True, False], p=prob):
      ...
      while len(episode_classes_rel) > self.max_ways_upper_bound:
        episode_classes_rel = RNG.choice(self.span_leaves_rel)
      ...
      if self.split == SPLIT_NAME_TO_SPLIT['train']:
          ...
          num_superclass_classes   = self.dataset_spec.episode_train_get_supclass_per_class_number(self.split,self.dataset_spec_dict,episode_superclass)
          ...
          supclass_select_class = self.dataset_spec.episode_train_get_supclass_per_class(self.split,self.dataset_spec_dict,episode_superclass)
          episode_select_classes = [supclass_select_class[i] for i in episode_subclass_ids]
          class_set_arr          = np.array(self.class_set)
          episode_classes_rel    = []
          for i in episode_select_classes:
              idx = np.where(class_set_arr==i)[0][0]
              episode_classes_rel.append(idx)
          # episode_classes_rel = supclass_select_class[episode_subclass_ids]
          # (episode_classes_rel,
      ...
      # ids_rel   = list(range(0,len(self.class_set)))
      ids_rel = [
      ...
      map_episode_classes = [class_id + self.class_set[0] for class_id in episode_classes_rel]
      class_set_arr       = np.array(self.class_set)
      new_episode_classes_rel = []
      for i in map_episode_classes:
        new_episode_classes_rel.append(np.where(class_set_arr == i)[0][0])

      episode_classes_rel = new_episode_classes_rel
