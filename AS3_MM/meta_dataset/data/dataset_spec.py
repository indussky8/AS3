...
def get_classes(split,classes_per_split,dataspec_dict):
...
class DatasetSpecification(
    collections.namedtuple('DatasetSpecification',
                           ('name, classes_per_split, images_per_class, '
                            'class_names,select_class, path, file_pattern'))):
  ...
  def get_classes(self, split,dataspec_dict):
    """Gets the sequence of class labels for a split.
    ...
    Raises:
      ValueError: An invalid split was specified.
    """
    return get_classes(split, self.classes_per_split,dataspec_dict)
  ...
  def to_dict(self):
    ...
    # select_class = []
    # for name in six.iteritems(ret_dict['select_class']):
    #     select_class.append(name)
    # print(ret_dict['select_class'])
    ret_dict['select_class'] = ret_dict['select_class']
    return ret_dict
...
class BiLevelDatasetSpecification(
  ...
  """The specification of a dataset that has a two-level hierarchy.
    ...
    Args:
      ...
      file_pattern: a string representing the naming pattern for each class's
        ...
        later into a big pool for sampling, as well as different splits that
        will be treated as disjoint pools for sampling the support versus query
        will be treaed as disjoint pools for sampling the support versus query
        examples of an episode.
  """
  ...
  def get_superclasses(self, split,):
    ...
    Raises:
      ValueError: An invalid split was specified.
    """
    num_classes = self.superclasses_per_split[split]

    if split == learning_spec.Split.TRAIN:
        offset = 0
    elif split == learning_spec.Split.VALID:
        offset = self.superclasses_per_split[learning_spec.Split.TRAIN]
    elif split == learning_spec.Split.TEST:
        offset = (
  ...
  def get_classes(self, split,dataspec_dict):
    ...
    if (self.restricted_classes_per_split is not None and
      ...
      if split == learning_spec.Split.TRAIN:
          ...
          num_classes    = self._count_classes_in_superclasses(self.get_superclasses(split))
          class_index = range(offset, offset + num_classes)
      elif split == learning_spec.Split.TEST:
          num_classes = self._count_classes_in_superclasses(self.get_superclasses(split))
          class_index = range(offset, offset + num_classes)
  ...
  def get_train_superclasses_perclasses_dict(self,split,dataspec_dict):

      ori_superclasses     = self.get_superclasses(split)
      superclass_per_class = dataspec_dict['classes_per_superclass']
      superclasses_perclasses_dict = {}
      total_class = 0
      for i in ori_superclasses:
          superclasses_perclasses_dict[i] = [total_class + i for i in range(superclass_per_class[i])]
          total_class += len(superclasses_perclasses_dict[i])

      return superclasses_perclasses_dict

  def get_class_to_superlclass_dict(self,split,dataspec_dict):

      if split == learning_spec.Split.TRAIN:
          # print(dataspec_dict.keys())
          ori_superclasses     = self.get_superclasses(split)
          # print(ori_superclasses)
          superclass_per_class = dataspec_dict['classes_per_superclass']
          # print(superclass_per_class)
          perclasses_superclass_dict = {}
          total_class = 0
          for i in ori_superclasses:
              for j in range(total_class,total_class+superclass_per_class[i]):
                perclasses_superclass_dict[j] = i
              total_class += superclass_per_class[i]


      return perclasses_superclass_dict

  def episode_train_get_supclass(self,split,dataspec_dict):
      if split == learning_spec.Split.TRAIN:
          # print(dataspec_dict.keys())
          ori_superclasses             = self.get_superclasses(split)
          superclasses_perclasses_dict = self.get_train_superclasses_perclasses_dict(split,dataspec_dict)
          select_class     = dataspec_dict['select_class']
          class_names_dict = dataspec_dict['class_names']
          class_names      = list(class_names_dict.values())
          # print(class_names)
          ...
          for i in class_index:
              for j in superclasses_perclasses_dict.keys():
                  if i in superclasses_perclasses_dict[j]:
                    if j not in superclass_list:
                        superclass_list.append(j)
                        break

      return superclass_list

  def episode_train_get_supclass_per_class_number(self,split,dataspec_dict,super_class):
      if split == learning_spec.Split.TRAIN:
          # print(dataspec_dict.keys())
          select_class     = dataspec_dict['select_class']
          class_names_dict = dataspec_dict['class_names']
          ...
          for i, name in enumerate(class_names):
              if name in select_class:
                  class_index.append(i)
          class_to_superclass_dict = self.get_class_to_superlclass_dict(split, dataspec_dict)


          total_class = 0
          for i in class_index:
              if class_to_superclass_dict[i] == super_class:
                  total_class += 1
  ...
  def episode_train_get_supclass_per_class(self,split,dataspec_dict,super_class):
      if split == learning_spec.Split.TRAIN:
          # print(dataspec_dict.keys())
          select_class     = dataspec_dict['select_class']
          class_names_dict = dataspec_dict['class_names']
...
class HierarchicalDatasetSpecification(
    collections.namedtuple('HierarchicalDatasetSpecification',
                           ('name, split_subgraphs, images_per_class, '
                            'class_names, path, file_pattern,select_class'))):
...
def load_dataset_spec(json_path,record_pth, convert_from_pkl=False):
  ...
  pkl_path = os.path.join(json_path, 'dataset_spec.pkl')
  ...
  records_pth = record_pth
  data_spec = data_spec._replace(path=records_pth)
